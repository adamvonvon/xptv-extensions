...
async function getTabs(token) {
    try {
        let list = []
        let url = appConfig.site + `/${token}/gayporn`
        ...
        for (const tyXml of tyMatches) {
            ...
            if (idMatch && titleMatch) {
                list.push({
                    name: titleMatch[1],
                    ext: {
                        url: `${appConfig.site}/${token}/gayporn?t=${idMatch[1]}&ac=videolist`,
                        token: `${token}`,
                    },
                })
            }
    ...
    try {
        ...
        for (const videoXml of videoMatches) {
            ...
            cards.push({
                ...
                ext: {
                    url: `${appConfig.site}/${token}/gayporn?ids=${idMatch[1]}`,
                },
            })
        }
...
async function search(ext) {
    try {
        ...
        const url = appConfig.site + `/${token}/gayporn?ac=videolist&wd=${text}`
        const { data } = await $fetch.get(url, {
            headers: {
                'User-Agent': UA,
            },
